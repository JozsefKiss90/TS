...
function classifyFailure(err: unknown): GatewayFailure {
  // ── TODO (Part B) ──────────────────────────────────────────────────
  // Turn each SDK error class into the matching GatewayFailure, most
  // specific first (the same chain exercise 02 wrote in its catch —
  // open that client.ts if you need the reference):
  //
  //   APIUserAbortError   → { kind: "aborted" }
  //   RateLimitError      → { kind: "throttled", retryAfterMs: ... }
  //                         (read err.headers?.get("retry-after") —
  //                          seconds on the wire, ms in the port; null
  //                          when the header is absent)
  //   APIConnectionError  → { kind: "transport", detail: err.message }
  //                         (this also covers APIConnectionTimeoutError —
  //                          a timeout IS a connection failure)
  //   APIError            → { kind: "rejected", status: err.status ?? 0,
  //                           detail: err.message }
  //
  // Anything else is NOT the wire's failure — it is a bug in our own
  // process, and bugs must stay loud: rethrow it (the last line below
  // already does; keep it as the final fallthrough).
  //
  // Until you implement this, every SDK error crosses the port as an
  // exception — run `pnpm job b` and watch the supervisor crash on a
  // 429 it should have classified.
  // ───────────────────────────────────────────────────────────────────
